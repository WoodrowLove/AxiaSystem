...
persistent actor AIRouter {
    ...
    private transient var performanceTracker = PM.PerformanceTracker();
    
    // Policy metrics tracking
    private var policyEvaluationStore: [_PE.PolicyEvaluation] = [];
    private transient let policyEvaluations = Buffer.fromArray<_PE.PolicyEvaluation>(policyEvaluationStore);
    ...
    public shared(msg) func submit(request: AIRequest) : async Result.Result<Text, Text> {
        ...
        if (not CB.shouldAllowRequest(circuitBreaker)) {
            ...
            performanceTracker.recordLatency(startTime, "circuit_breaker_trip", false);
        ...
        switch (checkRateLimit(caller, now)) {
            case (#err(error)) {
                ...
                performanceTracker.recordLatency(startTime, "rate_limit_violation", false);
        ...
        // 4. Policy Engine evaluation (Q1-Q5 compliant)
        let defaultPolicies = _PE.getDefaultPolicies();
        let policyEvaluation = _PE.evaluateRequest(request, defaultPolicies);
        
        // Record policy evaluation for metrics
        policyEvaluations.add(policyEvaluation);
        
        switch (policyEvaluation.decision) {
            case (#Block) {
                auditAction("SUBMIT_BLOCKED", caller, request.correlationId, "Policy engine blocked request: " # Text.join(", ", policyEvaluation.reasoning.vals()));
                return #err("Request blocked by policy: " # Text.join(", ", policyEvaluation.reasoning.vals()));
            };
            case (#RequireMFA) {
                auditAction("SUBMIT_MFA_REQUIRED", caller, request.correlationId, "Policy engine requires MFA: " # Text.join(", ", policyEvaluation.reasoning.vals()));
                return #err("Multi-factor authentication required: " # Text.join(", ", policyEvaluation.reasoning.vals()));
            };
            case (#Hold) {
            ...
            case (#Proceed) {
                auditAction("SUBMIT_POLICY_APPROVED", caller, request.correlationId, "Policy engine approved: " # Text.join(", ", policyEvaluation.reasoning.vals()));
            };
        };
    ...
    public shared(msg) func poll(correlationId: Text) : async Result.Result<AIResponse, Text> {
        let caller = Principal.toText(msg.caller);
        let startTime = performanceTracker.startTimer("poll");
                ...
                switch (storedRequest.status) {
                    case (#Pending or #Processing) {
                        ...
                        if (elapsedMsNat > storedRequest.request.timeoutMs) {
                            ...
                            performanceTracker.recordLatency(startTime, "timeout", false);
                        ...
                        performanceTracker.recordLatency(startTime, "poll", false);
                        #err("Request still processing")
                    };
                    case (#Completed(response)) {
                        performanceTracker.recordLatency(startTime, "poll", true);
                    ...
                    case (#Failed(error)) {
                        ...
                        performanceTracker.recordLatency(startTime, "failure", false);
    ...
    // Performance metrics endpoint with P95/P99 latency data
    public query func performanceMetrics() : async {
        ...
        let perfMetrics = performanceTracker.getPerformanceMetrics();
        let cbHealth = CB.getHealthStatus(circuitBreaker);
        
        // Calculate policy metrics
        let policyMetricsData = _PE.generatePolicyMetrics(Buffer.toArray(policyEvaluations));
        let auditLogArray = Buffer.toArray(auditLog);
        let piiViolations = auditLogArray.size(); // Simplified - would filter for PII violations in production
        
        {
            latency = {
                p50 = perfMetrics.requestLatency.p50;
                p90 = perfMetrics.requestLatency.p90;
                p95 = perfMetrics.requestLatency.p95;
                p99 = perfMetrics.requestLatency.p99;
                max = perfMetrics.requestLatency.max;
                min = perfMetrics.requestLatency.min;
                avg = perfMetrics.requestLatency.avg;
                count = perfMetrics.requestLatency.count;
            };
            throughput = {
                requestsPerSecond = perfMetrics.throughput.requestsPerSecond;
                requestsPerMinute = perfMetrics.throughput.requestsPerMinute;
            };
            errorRates = {
                circuitBreakerTrips = perfMetrics.errorRates.circuitBreakerTrips;
                rateLimitViolations = perfMetrics.errorRates.rateLimitViolations;
                timeouts = perfMetrics.errorRates.timeouts;
                failures = perfMetrics.errorRates.failures;
            };
            circuitBreakerStatus = {
                state = switch (circuitBreaker.state) {
                    case (#Closed) { "closed" };
                    case (#Open) { "open" };
                    case (#HalfOpen) { "half-open" };
                };
                isHealthy = cbHealth.isHealthy;
                failureRate = cbHealth.failureRate;
                avgResponseTime = cbHealth.avgResponseTime;
                recommendation = cbHealth.recommendation;
            };
            policyMetrics = {
                totalEvaluations = policyMetricsData.totalEvaluations;
                proceedCount = switch (Array.find<(_PE.PolicyDecision, Nat)>(policyMetricsData.decisionBreakdown, func((decision, count)) = decision == #Proceed)) {
                    case (?(_, count)) count;
                    case null 0;
                };
                blockCount = switch (Array.find<(_PE.PolicyDecision, Nat)>(policyMetricsData.decisionBreakdown, func((decision, count)) = decision == #Block)) {
                    case (?(_, count)) count;
                    case null 0;
                };
                holdCount = switch (Array.find<(_PE.PolicyDecision, Nat)>(policyMetricsData.decisionBreakdown, func((decision, count)) = decision == #Hold)) {
                    case (?(_, count)) count;
                    case null 0;
                };
                mfaCount = switch (Array.find<(_PE.PolicyDecision, Nat)>(policyMetricsData.decisionBreakdown, func((decision, count)) = decision == #RequireMFA)) {
                    case (?(_, count)) count;
                    case null 0;
                };
                escalateCount = switch (Array.find<(_PE.PolicyDecision, Nat)>(policyMetricsData.decisionBreakdown, func((decision, count)) = switch decision { case (#Escalate(_)) true; case _ false })) {
                    case (?(_, count)) count;
                    case null 0;
                };
                averageConfidence = policyMetricsData.averageConfidence;
                escalationRate = policyMetricsData.escalationRate;
                fallbackRate = policyMetricsData.fallbackRate;
            };
            piiProtection = {
                totalScanned = auditLogArray.size();
                violationsBlocked = piiViolations; // Would implement proper filtering
                violationRate = if (auditLogArray.size() > 0) Float.fromInt(piiViolations) / Float.fromInt(auditLogArray.size()) else 0.0;
            };
            communicationHealth = {
                queueDepth = pendingRequests.size();
                avgProcessingTime = perfMetrics.requestLatency.avg;
    ...
    system func heartbeat() : async () {
        ...
        // 3. Circuit breaker health check - throttled warning (every 30 seconds)
        ...
        if (not healthStatus.isHealthy) {
            ...
            if (timeSinceLastWarning > 30_000_000_000) { // 30 seconds in nanoseconds
    ...
    system func preupgrade() {
        ...
        policyEvaluationStore := Buffer.toArray(policyEvaluations);
    };
