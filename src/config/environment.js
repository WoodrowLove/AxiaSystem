// AxiaSystem Environment Configuration (JavaScript)
...
export const getCurrentNetwork = () => {
  // Check environment variables first (Node.js environments)
  if (typeof global !== 'undefined' && global.process && global.process.env) {
    const envNetwork = global.process.env.DFX_NETWORK || global.process.env.NODE_ENV;
...
export const loadCanisterIds = async (network) => {
...
const getDefaultCanisterIds = (network) => {
...
export class CanisterManager {
  ...
  async createActorConfig(canisterName) {
    ...
    return {
      ...
      // Will be filled by the application when creating actors
      idlFactory: null
    };
  }
}
...
export const devUtils = {
  ...
  getEnvironmentInfo() {
    return {
      ...
      config: canisterManager.getNetworkConfig(),
      canisterIds: canisterManager.canisterIds
    };
  }
};
...
if (typeof window !== 'undefined') {
  window.AxiaCanisterManager = canisterManager;
  window.AxiaDevUtils = devUtils;
}
