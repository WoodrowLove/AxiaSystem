...
let transaction: TreasuryTransaction = {
    ...
    sender = Principal.fromText("2vxsx-fae"); // Use the null Principal or another appropriate value
    receiver = ?userId;
    amount = amount;
    tokenId = tokenId;
    description = description;
...
if (currentBalance >= totalRewards) {
    ...
    id = nextTransactionId;
    timestamp = Int.abs(Time.now()); // Convert Time to Nat
    sender = Principal.fromText("2vxsx-fae"); // Wrap in Some()
    receiver = ?Principal.fromText("2vxsx-fae"); // Wrap in Some()
    amount = totalRewards;
    tokenId = tokenId;
    description = description;
