...
module {
    ...
    public class PayoutManager(walletProxy: WalletCanisterProxy.WalletCanisterProxy, eventManager: EventManager.EventManager) {
        ...
        public func initiatePayout(
            ...
            if (Array.isEmpty(recipients) or Array.isEmpty(amounts)) {
        ...
        public func executePayout(payoutId: Nat): async Result.Result<(), Text> {
            let payoutOpt = Array.find(payouts, func(p) { p.id == payoutId });
            ...
            switch (payoutOpt) {
                ...
                case (?payout) {
                    ...
                    for (index in 0 to Array.size(payout.recipients) - 1) {
                        ...
                        switch (creditResult) {
                            case (#err(e)) {
                                ...
                                payouts := Array.map(payouts, func(p) {
                    ...
                    await emitPayoutEvent(#PayoutExecuted, #PayoutExecuted {
                        ...
                        recipients = Array.map(payout.recipients, Principal.toText);
                    });
