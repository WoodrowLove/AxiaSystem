      ...
      switch (Trie.find(wallets, userIdKey, Principal.equal)) {
        ...
        case null {
          ...
          switch userCheck {
            ...
            case (#ok(_)) {
              let newWallet = {
                id = Time.now(); // Using the current timestamp as the wallet ID
                owner = userId;
    ...
    public func updateBalance(userId: Principal, amount: Int): async Result.Result<Nat, Text> {
      ...
      switch (Trie.find(wallets, userIdKey, Principal.equal)) {
        case (?wallet) {
          ...
          if (newBalanceInt < 0) {
            #err("Insufficient balance for this operation.")
          } else {
            ...
            // Log the update to the token module if relevant
            if (amount > 0) {
  let tokenUpdate = await tokenProxy.mintTokens(userId, Int.abs(amount));
  switch tokenUpdate {
    case (#err(e)) { Debug.print("Token module update failed: " # e); };
    case (#ok(_)) {};
  };
};
    ...
    public func deleteWallet(userId: Principal): async Result.Result<(), Text> {
      ...
      switch (Trie.find(wallets, userIdKey, Principal.equal)) {
        case (?_) {
          ...
          // Notify the user module of wallet removal
          let userUpdate = await userProxy.detachWalletFromUser(userId);
          switch userUpdate {
            case (#err(e)) { Debug.print("User module update failed: " # e); };
            case (#ok(_)) {};
          };
  ...
  switch (Trie.find(wallets, userIdKey, Principal.equal)) {
    case (?wallet) {
      if (wallet.balance < amount) {
        ...
        let newBalance : Nat = wallet.balance - amount; // Explicit type annotation here
        let updatedWallet = { wallet with balance = newBalance };
        wallets := Trie.put(wallets, userIdKey, Principal.equal, updatedWallet).0;

        // Record the transaction
        let transaction: WalletTransaction = {
          id = Nat64.toNat(Nat64.fromIntWrap(Time.now()));
          amount = -(Int.abs(amount)); // Use Int.abs() instead of Int.fromNat()
          timestamp = Nat64.toNat(Nat64.fromIntWrap(Time.now()));
          description = "Debit transaction";
        };
        ignore await recordTransaction(userId, transaction);
...
let transaction: WalletTransaction = {
  id = Nat64.toNat(Nat64.fromIntWrap(Time.now()));
  amount = Int.abs(amount);  // For credit, we don't need to negate the amount
  timestamp = Nat64.toNat(Nat64.fromIntWrap(Time.now()));
...
public func attachTokenBalance(
  ...
  switch tokenUpdate {
    ...
    case (?_wallet) {
        let transaction: WalletTransaction = {
            id = Nat64.toNat(Nat64.fromIntWrap(Time.now()));
            amount = -(Int.abs(amount));  // Assuming amount is already an Int
            timestamp = Nat64.toNat(Nat64.fromIntWrap(Time.now()));
...
public func getWalletBalance(userId: Principal): Result.Result<Nat, Text> {
  let userIdKey = { key = userId; hash = Principal.hash(userId) };
